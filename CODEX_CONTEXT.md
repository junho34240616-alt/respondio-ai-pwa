...
- 테스트 모드: 실제 로그인 없이 운영 경로 검증용 `live-mock` 리뷰 반환
...
- [x] 실제 리뷰 수집 경로 + crawler 테스트 모드(`live-mock`) 정리
...
- [x] 플랫폼 연결 메타데이터 확장 (`auth_mode`, `session_status`, `session_connected_at`, `session_last_validated_at`)
- [x] 크롤러 세션을 `platform + store_id` 단위로 분리하는 토대
...
- 배민 연결 기본 방향은 `원격 서버 브라우저 직접 로그인 + 세션 저장`이며, 크롤러는 저장된 세션 복원 후 실제 수집/등록을 수행
...
3. **실제 크롤링 고도화**: 플랫폼 연결 UX 정리, 외부 크롤러 배포 환경 정리
   - 무료 운영 기준으로는 crawler 세션을 로컬 디스크가 아니라 D1에 암호화 저장/복원하는 방향 유지
...
8. **프론트엔드 구조 정리**: `src/index.tsx` 분리, 공통 UI/스크립트 추출
9. **관리자 운영 기능 확장**: 금칙어 CRUD, 플랜 CRUD, 결제 운영, 실패 재처리 정교화
10. **후순위 확장**: R2 screenshot/html dump, Redis 큐, AI/post worker 분리, Next.js 전환 검토
