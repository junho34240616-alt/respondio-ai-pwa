...
async function getCurrentStoreId(page) {
  const result = await page.evaluate(async () => {
    const auth = JSON.parse(localStorage.getItem('respondio_auth') || 'null')
  ...
  if (result.status !== 200 || !result.body?.store?.id) {
    throw new Error(`Failed to resolve current store id: ${result.body?.error?.message || result.status}`)
  }

  return Number(result.body.store.id)
}

async function seedDirectSessionConnection(page, platform, platformStoreId, storeId) {
  const labels = {
    baemin: '배달의민족',
    coupang_eats: '쿠팡이츠',
    yogiyo: '요기요'
  ...
  const crawlerResponse = await fetch(`${crawlerUrl}/login`, {
    method: 'POST',
    headers: {
  ...
  if (!crawlerResponse.ok || !crawlerPayload?.success) {
    throw new Error(`Failed to prime crawler session for ${platform}: ${crawlerPayload?.error || crawlerPayload?.message || crawlerResponse.status}`)
  }

  const result = await page.evaluate(async ({ platformName, nextStoreId }) => {
    const auth = JSON.parse(localStorage.getItem('respondio_auth') || 'null')
    const response = await fetch(`/api/v1/platform_connections/${platformName}/session-state`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${auth?.access_token || ''}`
  ...
  }, { platformName: platform, nextStoreId: platformStoreId })

  if (result.status !== 200 || !result.body?.success) {
    throw new Error(`Failed to sync ${platform} direct session: ${result.body?.error?.message || result.body?.message || result.status}`)
  }

  await page.reload({ waitUntil: 'domcontentloaded' })
  await page.locator('#platform-connection-list').filter({ hasText: labels[platform] }).waitFor({ timeout: 15_000 })
  await page.locator('#platform-connection-list').filter({ hasText: labels[platform] }).filter({ hasText: '세션 활성' }).waitFor({ timeout: 15_000 })
}
...
async function main() {
  ...
  try {
    ...
    const storeId = await getCurrentStoreId(page)
    await seedDirectSessionConnection(page, 'baemin', 'BM-LIVE-001', storeId)
    await seedDirectSessionConnection(page, 'coupang_eats', 'CP-LIVE-001', storeId)
    await seedDirectSessionConnection(page, 'yogiyo', 'YG-LIVE-001', storeId)
