...
app.use((req, res, next) => {
  ...
  if (!CRAWLER_SHARED_SECRET) {
    return res.status(503).json({ error: 'Crawler shared secret is not configured' });
  }
...
function isSessionActive(session) {
  if (session?.mockMode && session?.loggedIn) {
    return true;
  }
...
async function createPlatformSession(platform, storeId) {
  ...
  const sessionData = {
    ...
    remoteAuthBootstrapPending: false,
    remoteAuthBootstrapFinishedAt: null
  };
...
function isBlankRemoteAuthUrl(url) {
  const normalized = String(url || '').trim().toLowerCase();
  return !normalized || normalized === 'about:blank';
}

async function waitForMeaningfulRemoteAuthSurface(session, timeoutMs = REMOTE_AUTH_INITIAL_SURFACE_WAIT_MS) {
  ...
  while (Date.now() < deadline) {
    ...
    if (!isBlankRemoteAuthUrl(session.page.url())) {
      return true;
    }

    const hasVisibleContent = await session.page.evaluate(() => {
      ...
      return !!body.querySelector('input, textarea, button, a, iframe, img, svg, canvas');
    }).catch(() => false);
  ...
  return !isBlankRemoteAuthUrl(session.page.url());
}
...
async function captureRemoteAuthScreenshot(session, options = {}) {
  ...
  if (!session.lastScreenshot && (session.remoteAuthBootstrapPending || isBlankRemoteAuthUrl(session.page.url()))) {
    await waitForMeaningfulRemoteAuthSurface(session);
  }
...
function isLoginSuccessUrl(platform, currentUrl) {
  ...
  if (platform === 'coupang_eats') {
    return (
      normalized.includes('store.coupangeats.com') &&
      !normalized.includes('/login') &&
  ...
  if (platform === 'yogiyo') {
    return (
      normalized.includes('ceo.yogiyo.co.kr') &&
      !normalized.includes('/login') &&
      !normalized.includes('/signin') &&
      !normalized.includes('/auth')
    );
...
async function hasVisibleLoginSurface(page, config, timeout = 1200) {
  ...
  for (const selector of probes) {
    const found = await probeVisibleSelectorInFrames(page, selector, timeout);
...
async function openLoginSurface(platform, page, config, options = {}) {
...
async function bootstrapRemoteAuthSession(remoteSession) {
  ...
  if (!isSessionActive(session)) {
    ...
    remoteSession.lastError = '원격 브라우저 세션이 초기화 중 종료되었습니다.';
    remoteSession.updatedAt = Date.now();
    return;
  }

  remoteSession.status = 'pending';
  remoteSession.lastError = null;
  remoteSession.updatedAt = Date.now();
  session.remoteAuthBootstrapPending = true;
  ...
  try {
    ...
    remoteSession.status = 'error';
    remoteSession.lastError = error.message || '원격 인증 화면을 열지 못했습니다.';
    remoteSession.updatedAt = Date.now();
    prewarmRemoteAuthScreenshot(session, 0);
  } finally {
    session.remoteAuthBootstrapPending = false;
    session.remoteAuthBootstrapFinishedAt = Date.now();
  }
}
...
async function getRemoteAuthSnapshot(remoteSession) {
  ...
  const looksAuthenticated = isLoginSuccessUrl(remoteSession.platform, currentUrl);
  const loginSurfaceVisible = looksAuthenticated
    ? await hasVisibleLoginSurface(session.page, PLATFORMS[remoteSession.platform], 450).catch(() => false)
    : false;
  const canComplete = looksAuthenticated && !loginSurfaceVisible;
  ...
  if (remoteSession.lastError) {
    stage = 'error';
    message = remoteSession.lastError;
  } else if (remoteSession.status === 'pending' && (!currentUrl || currentUrl === 'about:blank')) {
    stage = 'pending';
...
app.post('/remote-auth/start', async (req, res) => {
  ...
  try {
    ...
    bootstrapRemoteAuthSession(remoteSession).catch((error) => {
      remoteSession.status = 'error';
      remoteSession.lastError = error.message || '원격 인증 화면을 준비하지 못했습니다.';
      remoteSession.updatedAt = Date.now();
    });
...
function readRemoteAuthScope(req) {
  const platform = String(req.query.platform || req.body?.platform || '').trim();
  const rawStoreId = req.query.store_id ?? req.body?.store_id;
  const storeId = rawStoreId === undefined || rawStoreId === null || rawStoreId === ''
    ? null
    : normalizeStoreId(rawStoreId);

  return { platform, storeId };
}

function resolveScopedRemoteAuthSession(req, res) {
  const remoteSession = remoteAuthSessions.get(req.params.sessionId);
  if (!remoteSession) {
    res.status(404).json({ success: false, error: '원격 인증 세션을 찾을 수 없습니다.' });
    return null;
  }

  const scope = readRemoteAuthScope(req);
  if (!scope.platform || !scope.storeId) {
    res.status(400).json({ success: false, error: 'platform과 store_id가 필요합니다.' });
    return null;
  }

  if (remoteSession.platform !== scope.platform || remoteSession.storeId !== scope.storeId) {
    res.status(404).json({ success: false, error: '원격 인증 세션을 찾을 수 없습니다.' });
    return null;
  }
...
app.get('/remote-auth/:sessionId/status', async (req, res) => {
  const remoteSession = resolveScopedRemoteAuthSession(req, res);
  if (!remoteSession) {
    return;
  }
...
app.post('/remote-auth/:sessionId/complete', async (req, res) => {
  ...
  try {
    ...
    await persistPlatformSessionState(session).catch((error) => {
      remoteSession.status = 'error';
      remoteSession.lastError = `세션 저장 실패: ${error.message}`;
      remoteSession.updatedAt = Date.now();
      throw new Error(remoteSession.lastError);
    });
